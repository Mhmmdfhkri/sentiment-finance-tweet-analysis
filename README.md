...
*   **Pipeline Data:**
    *   Mengintegrasikan ekosistem **Hugging Face** untuk pemuatan dataset langsung via URL `hf://` dan tokenisasi cepat menggunakan `BertTokenizerFast` / `AutoTokenizer`.
    *   Memanfaatkan **PyTorch DataLoader** untuk melakukan *batching* (ukuran batch: 32) dan melakukan *shuffling* data latih secara acak menggunakan `RandomSampler`.
*   **Optimasi & GPU Accelerated:** Dilengkapi dengan pengecekan otomatis perangkat keras GPU (CUDA) untuk mempercepat proses training BERT, lengkap dengan penanganan *exploding gradients* menggunakan teknik *Gradient Clipping* (`clip_grad_norm_`).
